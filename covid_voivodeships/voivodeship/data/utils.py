...
def fill_missing_dates(df, from_date=DEFAULT_START_DATE):
    ...
    missing_rows = pd.DataFrame(index=missing_values, columns=df.columns, dtype=np.float)
...
def normalize_by_population(vol, df, per_100k=True):
    ...
    #print(df.dtypes)
    coeff = 1e5 if per_100k else 1
    if isinstance(df, pd.DataFrame) or isinstance(df, pd.Series):
        return df.mul((1/population_count) * coeff)
    else:
        return df * (1 / population_count) * coeff