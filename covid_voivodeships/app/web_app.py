...
from covid_voivodeships.app.geo import get_geo_df, \
    get_map, load_geojson, show_id_to_voiv_name
from covid_voivodeships.app.ids import IDS
from covid_voivodeships.app.layout.graphs import hover_map_graph,\
    graph_day_cases_per_week
...
from covid_voivodeships.plots.daily_cases import plot_single_voiv,\
    day_change_pie_chart, get_separate_voiv_plots,\
    avg_cases_per_days_per_population
...
DS_WITH_RECOVERY_CASES = False

ps = ProjectDataStructure()
voivodeship_page_urls, voivodeship_names = load_voiv_urls_with_names()
voivs = load_all_vols(ps, latest=True)
if voivs is None:  # Download data for the first time, if it is no data
    voivs = download_data(voivodeship_names, voivodeship_page_urls,
                          verbose=True)
    save_all_vols(ps, voivs)


# Layout and data preparation:
...
def update_data(n_clicks):
    ...
    if n_clicks is not None and n_clicks > 0:
        ...
        voivs = download_data(voivodeship_names, voivodeship_page_urls, verbose=True)
        save_all_vols(ps, voivs)
        print('Updated')
...
# TODO: Change globals to some other solution
...
def update_plots_grid(normalize, pathname, moving_mean, columns,
                      start_date, end_date, with_shared_yaxis_lim):
    # Function is adjusted to prevent from repeated updates, the problem is
    # known and it causes many the same updated (2 times or many more).
    ctx = dash.callback_context
    global LAST_CHANGE
    global LAST_CHANGE_OUTPUT
    if LAST_CHANGE == ctx.triggered:
        print("Fix repeated update", ctx.triggered, file=sys.stderr)
    ...
    plots = get_separate_voiv_plots(voivs, voivodeship_names, daily_change,
                                    normalize != 0, moving_mean, columns,
                                    with_filled_chart, start_date, end_date,
                                    with_shared_yaxis_lim)
...
@app.callback([Output(IDS.MAP_HOVER_HEADER, 'children'),
               ...
               Output(IDS.Badges.INFECTED, 'children'),
...
def map_hover_data(hover_data, pathname, normalize, moving_mean):
    ...
    plot = plot_single_voiv(voiv, daily_change=daily_change,
                            moving_mean=moving_mean, normalize=normalize != 0)
    ...
    if DS_WITH_RECOVERY_CASES:
        ...
        badges = [state.Zarazeni.values[0],
                  state.Zgony.values[0]]
...
def update_options_checklist(pathname):
    ...
    return return_list, only_value_from_dict(return_list)
